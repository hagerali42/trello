...
const router = Router()
//1- Add a new task
router.post('/',validation(validator.addTask),auth, taskController.addTask )

// 2-update task (title , description , status) and assign task to other user(user must be logged in) (creator only can update task)
router.put('/:taskId',validation(validator.updateTask),auth, taskController.updateTask )

// 3-delete task(user must be logged in) (creator only can delete task)
router.delete('/:taskId',validation(validator.deleteTask),auth, taskController.deleteTask )
...
router.get('/taskAdmain',auth, taskController.getAllCreatedTask )
